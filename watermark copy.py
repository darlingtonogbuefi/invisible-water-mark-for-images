...
def encode(image_path, output_path, watermark_text, spread=5, delta=2):
    """
    ...
    Args:
        ...
        spread: Number of consecutive blocks per bit.
    ...
    for i in range(0, h, 8):
        for j in range(0, w, 8):
            ...
            coeff = dct_block[4,3]
            # Embed bit by increasing/decreasing coefficient
            if bit == '1':
...
def decode(image_path, spread=5, threshold=1.0):
    """
    Decode watermark from JPEG image.

    Args:
        image_path: Path to watermarked image.
        spread: Number of consecutive blocks per bit (must match encode).
...
def decode(image_path, threshold=1.0):
    ...
    if metadata and "original_shape" in metadata:
        ...
        spread = metadata.get("spread", 5)
        y = cv2.resize(y, (w_orig, h_orig), interpolation=cv2.INTER_CUBIC)
    else:
        spread = 5  # default if metadata missing
